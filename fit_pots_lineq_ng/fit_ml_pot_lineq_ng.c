...
int main(int argc,char *argv[]){
  ...
  for( jr=0; jr<ngrid; jr++ ){
    ...
    if( strcmp(hemisphere,"south")==0 ){ 
      if( grid[jr].lat[0]==max_lat )nlon0++;
    }else{
  ...
  fprintf(stderr,"NULLS assinged\n");

  
  if( argc<2 ){
    fprintf(stderr,"Must specify velocity file name (vel_out)\n");
    exit(1);
  }
  ...
  fname=argv[argc-1];
  vf=fopen(fname,"r");
  fprintf(stderr,"before read_fit_record\n");
  ...
  fprintf(stderr,"before pot_file\n");
  char pot_file[256];
  ...
  for( j=0; j<vDstr.npts; j++ ){
    ...
    /* fprintf(stderr,"in loop %d %d %d %d %lf\n",j,indx,jeqn,nrows,vDstr.lats[j]); */
  ...
  while(1){
	...
	en=-vDstr.ve[j]*b_mag[indx]/1e9;
      ...
      indx_r=neighbors[indx].right[0];
      dlon_l=fabs(grid[indx].lon[1]-grid[indx].lon[0]);
